  ...
  return (
    <div className="w-full">
      ...
      <label
        ...
        `}
      >
        <input id={id} type="file" className="hidden" accept=".xlsx" onChange={handleFileChange} />
        {fileName ? (
          <div className="text-center">
             <Icon name="check" className="w-12 h-12 mx-auto text-green-500" />
            <span className="font-medium text-slate-600 mt-2 block break-all">{fileName}</span>
            <button onClick={handleClear} className="text-sm text-red-500 hover:underline mt-1">Clear</button>
          </div>
        ) : (
